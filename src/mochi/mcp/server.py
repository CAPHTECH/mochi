...
class MochiMCPServer:
    ...
    TOOLS: list[ToolDefinition] = [
        ...
        ToolDefinition(
            ...
            description=(
                "Generate code pattern suggestions for kiri-style implementation. "
                "Provide a goal description and optional code context. "
                "Returns pattern examples based on learned kiri conventions. "
                "NOTE: Suggestions show APPROACH and STRUCTURE, not exact APIs. "
                "Verify implementation details against actual kiri codebase."
            ),
            input_schema={
                "type": "object",
                "properties": {
                    ...
                    "goal": {
                        "type": "string",
                        "description": "What you want to implement (e.g., 'DuckDB query handler')",
                    },
                },
                "required": ["goal"],
            },
        ...
        ToolDefinition(
            name="generate_diff",
            description=(
                "Generate a unified diff for a code change in kiri style. "
                "Instead of generating full code, outputs only the changed parts. "
                "Useful for incremental modifications and code reviews. "
                "Returns unified diff format (--- +++ @@ lines)."
            ),
            input_schema={
                "type": "object",
                "properties": {
                    ...
                    "change_description": {
                        "type": "string",
                        "description": "Description of the desired change (e.g., 'Add error handling', 'Use async/await')",
                    },
                    "language": {
                        "type": "string",
                        "description": "Programming language (default: typescript)",
                    ...
                    "context": {
                        "type": "string",
                        "description": "LSP context with available methods/types (helps ensure correct APIs)",
                    },
                },
    ...
    def handle_tools_call(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        elif tool_name == "suggest_pattern":
            return self._handle_suggest_pattern(arguments)
        elif tool_name == "generate_diff":
            return self._handle_generate_diff(arguments)
        else:
            return {
                "content": [
                    {
    ...
    def _handle_suggest_pattern(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            # Use the model to suggest patterns
            ...
            result = self.engine.generate(
                instruction=instruction,
                input_text="List relevant patterns with examples.",
                max_new_tokens=1024,
            )

            # Parse as patterns (simple text for now)
            patterns = [
                {
    ...
    def _handle_generate_diff(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            result = self.engine.generate_diff(
